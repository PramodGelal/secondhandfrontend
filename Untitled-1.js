...
document.getElementById("messageBtn").addEventListener("click", () => {
  ...
  fetch(`http://localhost:8080/api98/payments98/search98/by-payer98?payerEmail=${encodeURIComponent(email_buyer)}`)
    .then(response => response.json())
    .then(data => {
      const groupedBySeller = data.reduce((acc, payment) => {
        const seller = payment.seller_email || 'Unknown Seller';
